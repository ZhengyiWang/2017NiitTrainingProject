...
public class ClassDao {
	...
	public int add(Class grade){
		String sql = "INSERT INTO class" + "(className,majorId,grade,monitor,monitor_connection)" + "VALUES(?,?,?,?,?)";
		int result = runner.update(sql,grade.getClassName(),grade.getMajorId(), grade.getGrade(),grade.getMonitor(), grade.getMonitor_connection());
		return result;
	}
	...
	public int update(Class grade){
		String sql = "UPDATE class SET " + "className=?,grade=?,monitor=?,monitor_connection=?,majorId=? WHERE classId=?";
		int result = runner.update(sql,grade.getClassName(), grade.getGrade(),grade.getMonitor(), grade.getMonitor_connection(),grade.getMajorId(),grade.getClassId());
		return result;
	}
